...
def get_configs_barotraumadir(barotraumadir):
    path_arr = []
    configs_barotraumadir = ["serversettings.xml", "config_player.xml", "LuaCsSetupConfig.xml", "hintmanager.xml", "creature_metrics.xml"]
    for config in configs_barotraumadir:
...
def backupBarotraumaData(barotrauma_dir, mods_dir, save_dir, backup_dir, max_saves):
    if os.path.exists(backup_dir):
        ...
        if len(directory_contents) >= int(max_saves):
